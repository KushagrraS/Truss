  ...
  return (
    <ThemeProvider value={theme}>
      <Select
        ...
        className="border-none font-rigidSquare text-sm"
      >
        {countries.map((item, index) => (
          ...
          >
            <Image
              src={item.flags}
              alt={item.name}
              className="!relative !h-6 !w-8 !object-contain"
              fill={true}
              sizes="100%"
            />
            <Box component={'text'} className="xsm:invisible md:invisible">{item.name}</Box>
          </Option>
        ))}
      </Select>
    </ThemeProvider>
